...
/*
cat /Users/lavya810/traces/equinix-sanjose.dirA.20090917-130200.0 | awk -F, '{per[$1]++;} END{for (key in per) {if (per[key] > 100) {print key,per[key];};}}'
*/

int main() {
  TaskHeavyHittersRev hh;
  DataPlane dp;
  hh.getHashSeedsFromDataPlane(dp);
  hh.getManglerFromDataPlane(dp);
  ...
  hh.setUserPreferencesDirectly(field, numRows, countersPerRow, threshold);
  ...
  for (int i = MINEPOCH; i <= MAXEPOCH; i++) {
	...
	if ( sscanf(cStringLine,"%d.%d.%d.%d,%d.%d.%d.%d,%d,%d,%d",&srcip1,&srcip2,&srcip3,&srcip4,&dstip1,&dstip2,&dstip3,&dstip4,&sport,&dport,&proto) == 11) {
      ...
      vector<int> heavyHitters;
      hh.getHeavyHitters(heavyHitters);
      vector<int>::iterator it;

      for (it = heavyHitters.begin(); it != heavyHitters.end(); it++)
	{
	  char tmp[20];
